...
def register_ipcluster(data):
    ...
    data._ipcluster["cluster_id"] = "mess-cli-"+str(os.getpid())
...
def get_client(cluster_id, profile, engines, timeout, cores, quiet, **kwargs):
    ...
    try: 
        ...
        if (engines == "MPI") or ("mess-cli-" in cluster_id):
            if not quiet:
...
NO_IPCLUSTER_CLI = """\
    ...
    ipcluster by hand and then pass the `--ipcluster` flag to MESS. See
    the docs for more info: http://pymess.rtfd.io/
    """
NO_IPCLUSTER_API = """
    No ipcluster instance found. See documentation for the proper way to set 
    up an ipcluster instance when running the MESS Python API. In short, 
